...
import com.atay.iz.tracking.TrackingCoordinator
...
class RideWeatherManagerTest {
    ...
    @Test fun incompleteForecastCannotPublishWatchBelowThresholdState() = test {
        ...
        assertEquals(com.atay.iz.wearprotocol.WearWeatherStatus.ERROR,watch.status)
    ...
    @Test fun fractionalProgressEndpointHazardCanPublishEveryWatchVersion() = test {
        ...
        val snapshot = com.atay.iz.wearprotocol.WearSnapshot(
            generatedAt = clock.get(), journeyId = first.id,
            mode = com.atay.iz.wearprotocol.WearMode.MOTORCYCLE,
            startedAt = first.startedAt, recording = true, weather = weather)
        for (version in 1..3) {
            val bytes = com.atay.iz.wearprotocol.WearProtocol.encodeSnapshot(snapshot, version)
            assertNotNull(com.atay.iz.wearprotocol.WearProtocol.decodeSnapshot(bytes))
        }
    }
