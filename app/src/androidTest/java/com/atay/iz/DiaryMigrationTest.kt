...
class DiaryMigrationTest {
    ...
    @Test fun migratingVersionThreePreservesDiaryAndCreatesCascadingHealthTables() = runBlocking {
        ...
        helper.runMigrationsAndValidate(name, 5, true, DiaryDatabase.MIGRATION_3_4, DiaryDatabase.MIGRATION_4_5).close()
        val db = Room.databaseBuilder(context, DiaryDatabase::class.java, name).addMigrations(DiaryDatabase.MIGRATION_3_4, DiaryDatabase.MIGRATION_4_5).build()
        try {
            val dao = db.diaryDao()
    ...
    @Test fun migratingVersionOnePreservesDiaryRelationshipsAndDoesNotInventSteps() = runBlocking {
        ...
        helper.runMigrationsAndValidate(name, 5, true, DiaryDatabase.MIGRATION_1_2, DiaryDatabase.MIGRATION_2_3, DiaryDatabase.MIGRATION_3_4, DiaryDatabase.MIGRATION_4_5).close()
        val db = Room.databaseBuilder(context, DiaryDatabase::class.java, name)
            .addMigrations(DiaryDatabase.MIGRATION_1_2, DiaryDatabase.MIGRATION_2_3, DiaryDatabase.MIGRATION_3_4, DiaryDatabase.MIGRATION_4_5).build()
        try {
            val dao = db.diaryDao()
    ...
    @Test fun migratingVersionTwoRetainsGoogleIdentifiersLegacyReviewAndWalkingSteps() = runBlocking {
        ...
        helper.runMigrationsAndValidate(name, 5, true, DiaryDatabase.MIGRATION_2_3, DiaryDatabase.MIGRATION_3_4, DiaryDatabase.MIGRATION_4_5).close()
        val db = Room.databaseBuilder(context, DiaryDatabase::class.java, name).addMigrations(DiaryDatabase.MIGRATION_2_3, DiaryDatabase.MIGRATION_3_4, DiaryDatabase.MIGRATION_4_5).build()
        try {
            val dao = db.diaryDao()
    ...
    @Test fun migratingVersionFourPreservesOsmDiaryAndHealthAndStartsNewTablesEmpty() = runBlocking {
        ...
        helper.runMigrationsAndValidate(name, 5, true, DiaryDatabase.MIGRATION_4_5).close()
        val db = Room.databaseBuilder(context, DiaryDatabase::class.java, name).addMigrations(DiaryDatabase.MIGRATION_4_5).build()
        try {
