...
class OsmSafetyRulesTest {
    ...
    @Test fun callbackRejectsStateClientAndExpiredTransactionChanges() {
        ...
        assertTrue(OsmSafetyRules.validCallback(pending, "client", "random-state", "org.iz.navigation:/oauth2redirect", startedAt + 1000))
        assertFalse(OsmSafetyRules.validCallback(pending, "client", "wrong", "org.iz.navigation:/oauth2redirect", startedAt + 1000))
        assertFalse(OsmSafetyRules.validCallback(pending, "other", "random-state", "org.iz.navigation:/oauth2redirect", startedAt + 1000))
        assertFalse(OsmSafetyRules.validCallback(pending, "client", "random-state", "org.iz.navigation:/oauth2redirect/evil", startedAt + 1000))
        assertFalse(OsmSafetyRules.validCallback(pending, "client", "random-state", "org.iz.navigation:/oauth2redirect", startedAt + 600_001))
    }

    @Test fun actualCallbackUriMustUseTheRegisteredPathWithoutAuthorityOrFragment() {
        assertTrue(OsmSafetyRules.isExactRedirect("org.iz.navigation:/oauth2redirect?code=abc&state=xyz"))
        for (invalid in listOf(null, "org.iz.navigation:/oauth2redirect/evil?code=abc", "org.iz.navigation://evil/oauth2redirect?code=abc",
            "org.iz.navigation:/oauth2redirect#code=abc", "org.iz.navigation:/%6fauth2redirect?code=abc", "https://osm.org/oauth2redirect")) {
