...
class TrackingController(context: Context) {
    ...
    suspend fun stopAfterRestore() = TrackingCoordinator.mutex.withLock {
        ...
        (context.applicationContext as com.atay.iz.IzApplication).navigation.invalidateAfterDiaryReplacement()
        (context.applicationContext as com.atay.iz.IzApplication).weatherManager.stop()
        settings.suppressedActivity = settings.currentActivity
    ...
    companion object {
        ...
        internal fun transitionIntent(context: Context): PendingIntent = PendingIntent.getBroadcast(
            context, 4020, Intent(context, ActivityTransitionReceiver::class.java).setAction("com.atay.iz.ACTIVITY_TRANSITION"),
