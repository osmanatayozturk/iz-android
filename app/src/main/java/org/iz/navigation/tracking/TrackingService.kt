...
class TrackingService : Service() {
                ...
                TrackingCoordinator.mutex.withLock {
                    if (id == null) {
                        ...
                        for (location in result.locations.sortedBy { it.time }) {
                            ...
                            (applicationContext as org.iz.navigation.IzApplication).navigation.onAcceptedLocation(session.id,
                                org.iz.navigation.navigation.NavigationFix(
                                    org.iz.navigation.weather.WeatherCoordinate(accepted.sample.latitude, accepted.sample.longitude),
                    ...
                    for (location in result.locations.sortedBy { it.time }) {
                        ...
                        (applicationContext as org.iz.navigation.IzApplication).navigation.onAcceptedLocation(journey.id,
                            org.iz.navigation.navigation.NavigationFix(
                                org.iz.navigation.weather.WeatherCoordinate(point.latitude, point.longitude),
                                point.recordedAt, point.accuracy, point.speed,
                                if (location.hasBearing()) location.bearing.takeIf { it.isFinite() } else null))
                        (applicationContext as org.iz.navigation.IzApplication).weatherManager.onAcceptedLocation(
                            journey.id, org.iz.navigation.weather.WeatherCoordinate(point.latitude, point.longitude),
    ...
    private fun detachDiary(id: String) {
        ...
        filter = PositionFilter()
        locationClient.removeLocationUpdates(dwellCallback)
        (applicationContext as org.iz.navigation.IzApplication).weatherManager.onTrackingStopped(id)
    ...
    override fun onDestroy() {
        ...
        id?.let { TrackingCoordinator.clearPendingManualStart(it) }
        (applicationContext as org.iz.navigation.IzApplication).weatherManager.onTrackingStopped(id)
        (applicationContext as org.iz.navigation.IzApplication).navigation.onTrackingStopped(id)
    ...
    companion object {
        ...
        private const val ACTION_SESSION = "org.iz.navigation.tracking.SESSION"
        private const val ACTION_DETACH = "org.iz.navigation.tracking.DETACH"
        const val ACTION_START = "org.iz.navigation.tracking.START"
        const val ACTION_DETECTED = "org.iz.navigation.tracking.DETECTED"
        const val ACTION_STOP = "org.iz.navigation.tracking.STOP"
        const val ACTION_REFRESH = "org.iz.navigation.tracking.REFRESH"
